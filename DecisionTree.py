...
def deviation(value,parent_pos,parent_neg):
    ...
    #calculates the difference between the actual counts and the expected counts at each node
    # deviation is the ((actual positives - expected positives)**2 / expected positives) + ((actual negatives - expected negatives)**2 / expected negatives)
    # expected positives = parent positives * (child positives + child negatives) / (parent positives + parent negatives)
    # expected negatives = parent negatives * (child positives + child negatives) / (parent positives + parent negatives)
    ...
    actual_pos = value.pos
    actual_neg = value.neg

    deviation = ((actual_pos - expected_pos)**2 / expected_pos) + ((actual_neg - expected_neg)**2 / expected_neg)
...
def test_tennis_tree(filename):
    ...
    mydataset = DataSet(name=filename)

    train, test = train_test_split(mydataset, test_split=0.2)

    mytree = DecisionTreeLearner(train) 

    error = err_ratio(mytree, test)



    return (train,test,mytree,error)
...
def genSyntheticTestSet():
    ...
    restaurant = SyntheticRestaurantTest(100)
    data = restaurant
...
def train_restaurant_tree(trainSet, testSet, N=200):
    ...
    #this is when the tree is trained on all 200 samples
    OutputTree = DecisionTreeLearner(trainSet)
    error = err_ratio(OutputTree, testSet)

    # now calculate the minimum number of samples required to achieve the same error rate as the tree trained on all 200 samples
    for i in range(1,N):
        # split the trainSet into i samples
        test, train = train_test_split(trainSet, start=0, end=i)

        # create a new tree using i samples
        tree = DecisionTreeLearner(train)
        # calculate the error rate of the new tree
        new_error = err_ratio(tree, testSet)
        # if the error rate of the new tree is the same as the error rate of the tree trained on all 200 samples
        # then we have found the minimum number of samples required
        if new_error == error:
            samples_required = i
            break
    ...
    return(OutputTree,samples_required)
...
def train_tree(trainSet, testSet):
    ...
    # create a tree using the trainSet
    tree = DecisionTreeLearner(trainSet)
    # calculate the error rate of the tree
...
def genPruneTestSet():
    ...
    #create a dataset with 1000 examples using the SyntheticRestaurantPruneTest method
    restaurant = SyntheticRestaurantPruneTest(1000)
    data = restaurant
...
def prune_tree(tree,testSet):
    ...
    # if p_value is greater than 0.05 then prune the tree
    while p_value < 0.05:
        clear_counts(tree)
        p_value,delta,error_rate = evaluate(tree,testSet)
...
if __name__ == "__main__":
    filename = "./tennis"
    tennis_dataSet,tree = learn_tennis_tree(filename)
    error_rate = test_tennis_tree(filename)
    print("Error_rate ",error_rate)
    train_set = genSyntheticTrainSet()    
    test_set = genSyntheticTestSet() 
    restaurant_tree,errors = train_restaurant_tree(train_set,test_set) #task 3c

    tree,error_rate = train_tree(train_set,test_set) #task 3d
